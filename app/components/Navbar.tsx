...
import { Shield, Globe, ChevronDown, Check, Menu, X } from "lucide-react";
...
export default function Navbar() {
  ...
  const closeAll = () => { setMobileOpen(false); setLangOpen(false); };

  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 h-16 flex items-center justify-between px-4 sm:px-8 lg:px-10 bg-white/95 backdrop-blur border-b border-slate-100">
        {/* Logo */}
        <Link href="/" onClick={closeAll} className="flex items-center gap-2.5 no-underline shrink-0">
              ...
              <div className="absolute top-full mt-1.5 right-0 bg-white border border-slate-200 rounded-xl shadow-xl py-1 min-w-[130px] z-50">
                {LANGS.map((lang) => (
                  <button key={lang.code}
                    ...
                    <span>{lang.flag}</span>
                    <span>{lang.full}</span>
                    {locale === lang.code && <Check size={12} className="ml-auto text-cyan-500" />}
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="flex lg:hidden items-center gap-2">
          ...
          <div className="relative">
            <button onClick={() => setLangOpen(!langOpen)}
              className="flex items-center gap-1 text-xs text-slate-600 font-medium px-2 py-1.5 rounded-lg border border-slate-200 bg-white cursor-pointer">
              <span>{current.flag}</span>
              <span>{current.label}</span>
            </button>
            {langOpen && (
              <div className="absolute top-full mt-1.5 right-0 bg-white border border-slate-200 rounded-xl shadow-xl py-1 min-w-[120px] z-50">
                {LANGS.map((lang) => (
                  <button key={lang.code}
                    onClick={() => { setLocale(lang.code); setLangOpen(false); }}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-left hover:bg-slate-50 cursor-pointer ${
                      locale === lang.code ? "text-cyan-500" : "text-slate-600"
                    }`}>
                    <span>{lang.flag}</span>
                    <span>{lang.full}</span>
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)}
            className="w-9 h-9 flex items-center justify-center rounded-lg border border-slate-200 text-slate-600 cursor-pointer">
      ...
      {/* Mobile drawer */}
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden" onClick={closeAll}>
          <div className="absolute inset-0 bg-black/20 backdrop-blur-sm" />
          <div className="absolute top-16 left-0 right-0 bg-white border-b border-slate-100 shadow-xl px-4 py-4 flex flex-col gap-1"
            ...
            <div className="border-t border-slate-100 mt-2 pt-3 flex flex-col gap-2">
              <Link href="/contact" onClick={closeAll}
                className="px-4 py-3 rounded-xl text-sm font-medium text-slate-700 no-underline hover:bg-slate-50">
                {t.nav.contact}
              </Link>
              <Link href="/demo" onClick={closeAll}
                className="text-center text-sm font-semibold text-white px-4 py-3 rounded-xl no-underline"
                style={{ background: "linear-gradient(135deg,#0ea5e9,#14b8a6)" }}>
                {t.nav.requestDemo}
              </Link>
            </div>
