...
export default function SolutionsPage() {
  ...
  return (
    <>
      ...
      >
        <h1 className="text-5xl font-extrabold text-white mb-4 tracking-tight">{sp.title}</h1>
        <p style={{ color: "rgba(255,255,255,0.7)" }} className="max-w-xl mx-auto">{sp.subtitle}</p>
      </section>
        ...
        >
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              <span
                className="inline-block text-xs font-semibold text-white px-3.5 py-1.5 rounded-full mb-4"
                style={{ background: GRAD }}
              >
                {g.badge}
              </span>
              <h2 className="text-3xl font-extrabold text-slate-900 mb-2">{g.title}</h2>
              <p className="text-slate-500">{g.subtitle}</p>
            </div>
            <div
              className={`grid gap-5 ${
                g.cards.length === 4 ? "grid-cols-4" :
                g.cards.length === 2 ? "grid-cols-2 max-w-2xl mx-auto" : "grid-cols-3"
              }`}
            >
              {g.cards.map((c, ci) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="font-bold text-base text-slate-900 mb-1.5">{c.name}</h3>
      ...
      >
        <h2 className="text-3xl font-extrabold text-white mb-3">{sp.ctaTitle}</h2>
        <p className="text-slate-400 mb-10 max-w-md mx-auto">{sp.ctaSubtitle}</p>
