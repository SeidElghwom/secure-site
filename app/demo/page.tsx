...
export default function DemoPage() {
  const { t } = useLang();
  const d = t.demoPage;
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", company: "", country: "", industry: "", interest: "" });
  ...
  return (
    <>
      <section className="py-20 sm:py-28 px-4 sm:px-8 lg:px-12 text-center"
        ...
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto mb-6"
          style={{ background: "linear-gradient(135deg,#0ea5e9,#14b8a6)" }}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
            <path d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.35C17.25 22.15 21 17.25 21 12V7L12 2z" fill="white" />
          </svg>
        </div>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white mb-4">{d.title}</h1>
        <p style={{ color: "rgba(255,255,255,.7)" }} className="text-sm sm:text-base">{d.subtitle}</p>
      </section>

      <section className="py-12 sm:py-16 px-4 sm:px-8 lg:px-12" style={{ background: "#F4F7FA" }}>
        <div className="max-w-2xl mx-auto">
          {submitted ? (
            <div className="bg-white rounded-2xl p-10 sm:p-12 text-center border border-slate-100">
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-white text-2xl mx-auto mb-6"
                style={{ background: "linear-gradient(135deg,#0ea5e9,#14b8a6)" }}>✓</div>
              <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 mb-3">{d.successMsg}</h2>
            </div>
          ) : (
            <div className="bg-white rounded-2xl p-6 sm:p-10 border border-slate-100">
              <div className="flex flex-col gap-4 sm:gap-5">
                {/* First + Last on same row from sm up */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                  {[
                    { key: "firstName", label: d.firstNameLabel },
                    { key: "lastName",  label: d.lastNameLabel  },
                  ].map(({ key, label }) => (
                ...
                {[
                  { key: "email",   label: d.emailLabel,   type: "email" },
                  { key: "company", label: d.companyLabel, type: "text"  },
                ].map(({ key, label, type }) => (
                  <div key={key} className="flex flex-col gap-1.5">
                    <label className="text-sm font-medium text-slate-700">{label}</label>
                ...
                {[
                  { key: "country",  label: d.countryLabel,  placeholder: d.countryPlaceholder,  options: d.countries   },
                  { key: "industry", label: d.industryLabel, placeholder: d.industryPlaceholder, options: d.industries  },
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea rows={4} value={form.interest}
                ...
                <button onClick={handleSubmit}
                  className="w-full py-3 rounded-lg text-sm font-semibold text-white transition-opacity hover:opacity-90 cursor-pointer mt-1"
                  style={{ background: "linear-gradient(135deg,#0ea5e9,#14b8a6)" }}>
                  {d.submitBtn}
                </button>
              </div>
